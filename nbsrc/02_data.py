...
# %% export
import json, secrets
from awseasy.core import named, tag_list, wait_for
...
with mock_aws():
    ...
    db = create_postgres(auth, 'app-db', kms_key_id=key['Arn'], wait=True, **HIPAA)
...
with mock_aws():
    auth = AWSAuth(region='us-east-1')
    rg = create_redis(auth, 'app-cache', wait=True, **ISO27001)

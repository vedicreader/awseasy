...
def create_eks(auth, name, node_type='m5.large', node_count=3,
    ...
    cluster_role = create_role(auth, f'{name}-eks-cluster-role',
                               trust_policy=_eks_cluster_trust())
    attach_policy(auth, f'{name}-eks-cluster-role',
                  'arn:aws:iam::aws:policy/AmazonEKSClusterPolicy')

    node_role = create_role(auth, f'{name}-eks-node-role',
...
def create_ecr(auth, name, scan_on_push=True, tags=None) -> dict:
    'Create ECR repository with scan-on-push and AES256 encryption.'
    client = _ecr(auth)
...
def ecr_login_url(auth, name) -> str:
    'Return the ECR repository URI (use as docker push/pull target).'
